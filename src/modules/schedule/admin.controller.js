...
exports.createSchedule = async (req, res) => {
  try {
    ...
    if (scheduleCount >= 5) {
      return res.status(400).json({ message: 'Max 5 schedules allowed per day' });
    }
...
exports.assignTrainer = async (req, res) => {
  try {
    ...
    if (!trainer || trainer.role !== 'trainer') {
      ...
      return res.status(404).json({ message: 'Trainer not found or invalid role' });
    }
    ...
    res.status(200).json({ message: 'Trainer assigned successfully', schedule });
  } catch (err) {
